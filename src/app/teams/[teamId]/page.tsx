...
export default function TeamPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-6">
      <div className={`max-w-md w-full p-8 rounded-2xl shadow-2xl bg-gradient-to-br ${team.color} text-white text-center transform transition-all hover:scale-105`}>
        <h2 className="text-4xl font-extrabold mb-4">{team.name}</h2>
        <p className="text-lg mb-2">Team ID: <span className="font-semibold">{teamId}</span></p>
        <p className="text-lg mb-6">Number of Members: <span className="font-semibold">{team.members}</span></p>
        {/* Example buttons */}
